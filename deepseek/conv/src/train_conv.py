...
def parse_args():
    ...
    parser.add_argument(
        "--seed", type=int, default=22, help="A seed for reproducible training."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./prompt-conv-inspired",
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="max length of both encoder and decoder input.",
    )
    parser.add_argument(
        "--resp_max_length", type=int, default=80, help="max length of decoder input."
    )
    parser.add_argument(
        "--entity_max_length",
        type=int,
        default=64,
        help="max entity length in dataset.",
    )
    ...
    parser.add_argument(
        "--n_examples", type=int, default=3, help="number of retrieved demonstrations"
    )
    parser.add_argument(
        "--num_train_epochs",
        type=int,
    ...
    parser.add_argument(
        "--fp16", action="store_true", help="use automatic mixed precision to speed up."
    )
    parser.add_argument(
        "--mapping", action="store_true", help="if we use semantic mapping"
    )
    parser.add_argument(
        "--bias_only", action="store_true", help="if we use semantic mapping"
    )
    ...
    parser.add_argument(
        "--type_of_run",
        default="training",
        help="type of the experiment, eg: full, ablation, analysis",
    )
...
if __name__ == "__main__":
    ...
    logger.remove()
    logger.add(
        sys.stderr, level="DEBUG" if accelerator.is_local_main_process else "ERROR"
    )
    logger.add(
    ...
    if args.use_wandb:
        ...
        if args.log_all:
            ...
            if accelerator.is_local_main_process:
                run = wandb.init(
                    entity=args.entity, project=args.project, config=config, name=name
                )
            else:
                run = None
    else:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            prompt_augmented_input_embeddings, new_attention_mask, _, _ = (
                prompt_encoder(
                    ...
                    context_input_embeddings=model.get_input_embeddings()(
            ...
            pad_resp = (
                ...
                * torch.ones(
                    (
                        new_attention_mask.shape[0],
                        args.n_examples * args.prompt_max_length,
                    )
                )
                .to(new_attention_mask.device)
                .long()
            )
            #### padded response
            ...
            if (
                ...
                if run:
                    run.log(
                        {"loss": np.mean(train_loss) * args.gradient_accumulation_steps}
                    )
        ...
        model.eval()
        for batch in tqdm(
            valid_dataloader, disable=not accelerator.is_local_main_process
        ):
            with torch.no_grad():
                token_embeds = text_encoder(**batch["prompt"]).last_hidden_state
                ### compute the retrieval-augmented prompts
        ...
        for batch in tqdm(
            valid_gen_dataloader, disable=not accelerator.is_local_main_process
        ):
            with torch.no_grad():
                token_embeds = text_encoder(**batch["prompt"]).last_hidden_state
                ### compute the retrieval-augmented prompts
                ...
                for gen_seq, length in zip(gen_seqs, batch["context_len"]):
                    gen_seq = [
                        token_id.item()
                        for token_id in gen_seq
                        if token_id != tokenizer.pad_token_id
                    ]
                    gen_resp_ids.append(gen_seq)
                    # gen_resp_ids.append(gen_seq[length:])
                evaluator.evaluate(
                    gen_resp_ids, batch["resp"], log=accelerator.is_local_main_process
                )
        ...
        for batch in tqdm(
            test_gen_dataloader, disable=not accelerator.is_local_main_process
        ):
            with torch.no_grad():
                token_embeds = text_encoder(**batch["prompt"]).last_hidden_state
                ### compute the retrieval-augmented prompts
