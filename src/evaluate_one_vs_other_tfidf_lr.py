...
from train_one_vs_other_tfidf_lr import build_text
...
def import_dependencies():
    """Import sklearn/joblib và báo lỗi rõ nếu môi trường chưa cài đủ."""
    try:
        import joblib
...
def resolve_model_dir(model_dir: Path) -> Path:
    ...
    fallback_dir = PROJECT_ROOT / "models" / "one_vs_other"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate one-vs-other TF-IDF + Logistic Regression models."
    )
    parser.add_argument(
        "--model-dir",
        default=PROJECT_ROOT / "data" / "models",
        type=Path,
        help="Folder chứa 14 model .joblib. Mặc định: <project>/data/models.",
    )
    parser.add_argument(
        "--test-dir",
        default=PROJECT_ROOT / "data" / "test",
        type=Path,
        help="Folder chứa test set đã tách theo category. Mặc định: <project>/data/test.",
    )
    parser.add_argument(
        "--test-file",
        default=None,
