...
class DCMoMEModel(nn.Module):
    def __init__(
        ...
        self.dialogue_encoder = DialogueTurnEncoder(dialogue_backbone, config.encoder.dialogue_dim)
        self.prompt_text_backbone = dialogue_backbone
        self.projectors = nn.ModuleDict(
            {
                "kg": ModalityProjector(config.encoder.kg_dim, config.encoder.projector_hidden_dim, hidden_size),
                "text": ModalityProjector(config.encoder.text_dim, config.encoder.projector_hidden_dim, hidden_size),
                "visual": ModalityProjector(config.encoder.visual_dim, config.encoder.projector_hidden_dim, hidden_size),
            }
        )
        self.experts = nn.ModuleDict(
            {name: ModalityExpert(hidden_size, config.encoder.expert_hidden_dim) for name in ("kg", "text", "visual")}
        )
        ...
        self.register_buffer(
            "item_ids",
            torch.as_tensor(graph_bundle.item_ids, dtype=torch.long) if graph_bundle.item_ids else torch.zeros(0, dtype=torch.long),
        )
        if graph_bundle.text_feature_matrix is None:
            graph_bundle.text_feature_matrix = torch.zeros((graph_bundle.num_entities, config.encoder.text_dim), dtype=torch.float32)
        if graph_bundle.visual_feature_matrix is None:
            graph_bundle.visual_feature_matrix = torch.zeros((graph_bundle.num_entities, config.encoder.visual_dim), dtype=torch.float32)
        self.register_buffer("global_text_features", graph_bundle.text_feature_matrix.float())
        self.register_buffer("global_visual_features", graph_bundle.visual_feature_matrix.float())
    ...
    def forward(
        ...
        h_t = self._project_optional("text", batch.text_features, projected_like=kg_states)
        h_v = self._project_optional("visual", batch.visual_features, projected_like=kg_states)
        ...
        previous_turn = None
        if batch.previous_turn_input_ids is not None and batch.previous_turn_attention_mask is not None:
            previous_turn = self.dialogue_encoder(
                batch.previous_turn_input_ids,
                batch.previous_turn_attention_mask,
            )

        modality_mask = self._build_modality_mask(batch)
        current_routing = self.router(current_turn, projected, modality_mask=modality_mask)
        ...
        if not prompt_output_entity and batch.retrieved_prompt_input_ids is not None and batch.retrieved_prompt_attention_mask is not None:
    ...
    def _project_optional(
        ...
        projected_like: torch.Tensor,
    ) -> torch.Tensor:
        if features is None:
            batch_size, num_entities = projected_like.shape[:2]
            output_dim = self.config.prompt.hidden_size
            return projected_like.new_zeros((batch_size, num_entities, output_dim))
        return self.projectors[name](features)

    @staticmethod
    def _build_modality_mask(batch: DCMoMEBatch) -> dict[str, torch.Tensor]:
        entity_mask = batch.entity_mask
        masks = {"kg": entity_mask}
        masks["text"] = entity_mask if batch.text_features is None else entity_mask * (batch.text_features.abs().sum(dim=-1) > 0).long()
        masks["visual"] = entity_mask if batch.visual_features is None else entity_mask * (batch.visual_features.abs().sum(dim=-1) > 0).long()
        return masks
    ...
    def get_all_entity_embeds(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        entity_ids = torch.arange(all_kg.size(0), device=all_kg.device, dtype=torch.long)
    ...
    def encode_retrieved_prompt_tokens(
        ...
        hidden_states = hidden_states.contiguous().view(batch_size, n_examples * hidden_states.size(1), hidden_states.size(2))
