...
class CRSConvDataset(Dataset):
    def __init__(
        ...
        dataset_dir = os.path.join("conv_data", dataset)
    ...
    def prepare_data(self, data_file):
        with open(data_file, "r", encoding="utf-8") as f:
            ...
            for line in tqdm(lines):
                ...
                for c, r in list(zip(dialog["mm_contexts"], dialog["mm_resps"]))[
                    : self.n_examples
                ]:
                ...
                for i, sent in enumerate(retrieved_examples):
                    ids = self.prompt_tokenizer.convert_tokens_to_ids(
                        self.prompt_tokenizer.tokenize(sent)
                    )
                    ...
                    ids = self.tokenizer.convert_tokens_to_ids(
                        self.tokenizer.tokenize(retrieved_gen_examples[i])
                    )
                    ids = ids[-self.context_max_length :]
                ...
                data = {
                    ...
                    "entity": list(
                        ...
                        if all(
                            item is None
                            for item in dialog["retrieved_mm_response_entity"]
                        )
                        and all(
                            item is None
                            for item in dialog["retrieved_mm_context_entity"]
                        )
                        else (
                            dialog["retrieved_mm_response_entity"]
                            + dialog["retrieved_mm_context_entity"]
                            + dialog["entity"]
...
class CRSConvDataCollator:
    def __init__(
        ...
        self.n_examples = n_examples
        self.generate_prompt_ids = self.tokenizer.convert_tokens_to_ids(
        ...
        if self.gen:
            ...
            for data in data_batch:
                ...
                context_ids = context_ids[
                    -(self.context_max_length - len(self.generate_prompt_ids)) :
                ]
                ...
                retrieved_gen_batch["input_ids"].extend(
                    data["retrieved_example_gen_ids"]
                )
